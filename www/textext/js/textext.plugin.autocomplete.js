...
(function($)
{
	...
	function seleccion(valorSeleccion){
	  valorSeleccion=valorSeleccion;
	  $(".temp").val(valorSeleccion);
    } 
	...
	p.addDropdownItem = function(html)
	{
		...
		node.find('.text-label').click(function(){ seleccion(html);return true});
	...
	p.selectFromDropdown = function()
	{
		var self       = this,
			...
			;
		valorSeleccion=$(".temp").val();
		if (valorSeleccion!="") suggestion=valorSeleccion;
		$(".temp").val("");
		//alert("Pilla->"+valorSeleccion+"_"+suggestion);
		if(suggestion)
		{
