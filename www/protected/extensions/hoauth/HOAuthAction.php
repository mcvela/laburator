...
class HOAuthAction extends CAction
{
	...
	/**
	 * @var string $uxernameAttribute you can specify the username attribute, when user must fill it
	 */
	 ...
	 */
	 // 'birthDay',
	protected $_avaibleAtts = array('status','identifier', 'profileURL', 'webSiteURL', 'photourl', 'displayName', 'description', 'firstName', 'lastName', 'gender', 'language', 'age',  'birthMonth', 'birthYear', 'email', 'emailVerified', 'phone', 'address', 'country', 'region', 'city', 'zip', 'birthDate', 'genderShort');
	...
	protected function oAuth( $provider )
	{
		try{
			...
			else 
			{
				...
				//$this->processUser($user, $userProfile); // Actualizar la url de la foto
				
				
				$user->lastvisit=time();
				Yii::trace("lastvisit USUARIO >", "warm");
				if(!$user->save())
	...
	protected function processUser($user, $userProfile)
	{
		...
		// NOTE: we display `username` field in our form only if it is required by the model
		 if(!$user->isAttributeRequired($this->_emailAttribute)) //$this->usernameAttribute
			$this->_emailAttribute = false;

		 Yii::trace("this->usernameAttribute>".$this->usernameAttribute, "warm");
		 Yii::trace("this->usernameAttribute>".$this->_emailAttribute, "warm");
		
		$form = new HUserInfoForm($user, $this->_emailAttribute,''); //, $this->usernameAttribute

		
		
		Yii::trace("userProfile->photourl>".$userProfile->photourl, "warm");
	/*	if(!$form->validateUser())
		...
		if($user->isNewRecord) 
		{
		    Yii::trace("SAVE processUser--$user->status>".$user->status, "warm");
			$user->status=1;  /// Activar el usuario de una vez por todas
			...
			// MAAV ----------------> ENVIARME EMAIL DE BIENVENIDA
			...
			if($this->useYiiUser)
			{
				...
				//Yii::trace("PROFILE URL PHOTO>".$profile->hasAttribute('photoURL')."---->", "warm");

				//if($profile->hasAttribute('photoURL')) $profile->photoURL = $profile->getAttribute('photoURL');
	...
	protected function populateModel($user, $profile)
	{
		...
		{
		    Yii::trace("populateModel--REVISAR->".$pAtt, "warm");
			if(in_array($pAtt, $this->_avaibleAtts))
			{
			    Yii::trace("populateModel--LEO->".$pAtt, "warm");
				switch($pAtt)
				{
					case 'genderShort':
					...
					break;
					case 'photourl':
					$att = $profile->photourl;
					break;
					case 'status':
					$att = User::STATUS_ACTIVE;
					break;
					default:
					$att = $profile->$pAtt;
				}
				if(!empty($att)){
				     Yii::trace("populateModel-->".$pAtt.":".$att, "warm");
					$user->$attribute = $att;
					}
			}else{
				$user->$attribute = $pAtt;
